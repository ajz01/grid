...
func argbToRgba(argb string) string {
	if len(argb) < 3 {
		return argb
	}
...
func (g grid) draw() {
	...
	for i := range g.data {
		...
		if s.cell != nil {
			...
			if style != nil {
				fgColor = "#" + argbToRgba(style.Fill.FgColor)
				fontColor = "#" + argbToRgba(style.Font.Color)
			}
		}
		if len(fgColor) > 1 {
...
func (g *grid) addData(row, col int, value string, cl *xlsx.Cell) {
	...
	c := cell{x, y, row, col, value, false, cl}
...
func newGrid(obj gridObj) *grid {
	...
	keyDownCb := js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		...
		if g.editCell != nil {
			...
			if _, ok := g.data[address{ec.row, ec.col}]; !ok {
				g.data[address{ec.row, ec.col}] = ec
			}
			if c == "Tab" {
				ec.editing = false
	...
	vcnv.Call("addEventListener", "drop", drpCb)
	vcnv.Call("addEventListener", "dragover", drgCb)
